...
# 10 Most Populous Cal State Universities (by enrollment)
csu_campuses <- tribble(
  ~Campus,                            ~Longitude, ~Latitude,
  "CSU Fullerton",                    -117.8850,   33.8826,
  "CSU Northridge",                   -118.5289,   34.2381,
  "San Diego State University",       -117.0732,   32.7757,
  "CSU Long Beach",                   -118.1141,   33.7838,
  "San Jose State University",        -121.8826,   37.3352,
  "Cal Poly Pomona",                  -117.8195,   34.0584,
  "CSU Sacramento",                   -121.4246,   38.5610,
  "CSU Los Angeles",                  -118.1684,   34.0655,
  "CSU San Bernardino",               -117.3213,   34.1814,
  "Cal Poly San Luis Obispo",         -120.6605,   35.3000
)


uc_sf <- st_as_sf(csu_campuses, coords = c("Longitude", "Latitude"), crs = 4326) |> 
  st_transform(3310)
```
...
rent_joined <- rent_joined %>%
  mutate(
    Expected_2025 = RentIndex_2015 * 2.10748158,
    Diff_vs_expected = RentIndex_2025 - Expected_2025
  )
...
for (i in 1:nrow(uc_sf)) {
  ...
  p <- ggplot() +
    ...
    geom_sf(data = campus_pt, shape = 21, fill = "black", color = "white", size = 3) +
    labs(
      title = paste0("Raw Rent Growth vs Legal Limit near ", campus_name),
...
for (i in 1:nrow(uc_sf)) {
  ...
  buffer_1 <- st_buffer(campus_pt, 1609)
  buffer_3 <- st_buffer(campus_pt, 1609 * 3)
  buffer_5 <- st_buffer(campus_pt, 1609 * 9)
